...
export default function ProgressScreen() {
  ...
  const chooseScope = async (nextScope: ScoreScope) => {
    ...
    if (nextScope === 'all') {
      ...
      const unlockGeneration = requestGeneration.current; setBusy(true); const unlocked = await guard.current.unlock(); setBusy(false); if (!unlocked || !appActive.current || requestGeneration.current !== unlockGeneration || !session || !isCurrentAuthSession(session)) { guard.current.clear(); if (appActive.current && requestGeneration.current === unlockGeneration) setError('需要完成身分驗證才能查看全部。'); return; }
  ...
  return <View style={styles.screen}>
    <View style={styles.top}><View><Text style={styles.title}>積分</Text><Text style={styles.subtitle}>{scope === 'me' ? '我的積分' : scope === 'friends' ? '好友積分' : '完整排名'}</Text></View><Pressable accessibilityRole="button" accessibilityLabel="開啟積分操作" onPress={() => setSheet('menu')} style={styles.menu}><Text style={styles.menuText}>⋯</Text></Pressable></View>
    <View style={styles.scopes}>{(['me', 'friends', ...(capabilities?.canViewAllScores ? ['all'] : [])] as ScoreScope[]).map((item) => <Pressable key={item} accessibilityRole="tab" accessibilityState={{ selected: scope === item }} accessibilityLabel={item === 'me' ? '自己' : item === 'friends' ? '好友' : '全部'} onPress={() => void chooseScope(item)} style={[styles.scope, scope === item && styles.scopeActive]}><Text style={[styles.scopeText, scope === item && styles.scopeTextActive]}>{item === 'me' ? '自己' : item === 'friends' ? '好友' : '全部 🔒'}</Text></Pressable>)}</View>
    ...
    <ActionSheet visible={sheet === 'rewards'} title="選擇目標獎品" onClose={() => setSheet(null)}><RewardControls rewards={rewards} selectedRewardId={selectedRewardId} canEdit onSelect={(rewardId) => void setTarget(rewardId)} /></ActionSheet>
