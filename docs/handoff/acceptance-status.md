...
- 0.2.12/code14（8c29b89）真機：冷啟開讀經器，朗讀鈕直接為「播放」（0.2.9–0.2.11 曾因伺服器 6 h 快取超過 5 min validUntil 而每次顯示「重試」，logcat `rejected:EXPIRED`；修法＝快取壽命綁 validUntil、預熱每 4 min）；註腳面板（詩96:9）點灰底／「關閉」／返回鍵三種方式皆關閉且讀經器保留；積分頁上方合併為單一 compact 區塊。後端已同步快取修正。
