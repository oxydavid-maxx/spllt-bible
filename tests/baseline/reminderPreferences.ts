...
// Additive storage only. This deployment does not install a worker or a delivery provider.
...
export function readReminderPreferences(db: DatabaseSync, memberId: string) {
  ...
  return {
    ...
    remoteDeliveryStatus: 'REMOTE_PENDING' as const,
    // This deployed baseline contains RPG links, but no authoritative meeting schedules.
    meetings: [],
  };
}
