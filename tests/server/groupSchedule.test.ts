...
describe('member RPG meeting context', () => {
  it('returns the approved schedule, masked roster and revision for the member group', async () => {
    ...
    expect(response).toMatchObject({ status: 200, body: { groupId: 'G01', rpgs: [{ rpgId: 'G01-RPG1', meeting: { meetingId: 'm1', timeZone: 'Asia/Taipei', revision: 3, status: 'SCHEDULED' }, standingRoom: true }] } });
    database.close();
  });
});
